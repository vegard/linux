...
static void symbol_to_bool_expr(struct symbol *sym, struct bool_expr *result[2])
{
	...
	switch (sym->type) {
	case S_UNKNOWN:
		/* Undefined symbols can never have a true (=y/=m) value. */
		result[0] = bool_const(false);
		result[1] = bool_const(false);
		return;
	case S_BOOLEAN:
...
static void expr_to_bool_expr(struct symbol *lhs, struct expr *e, struct bool_expr *result[2])
{
	...
	case E_LEQ:
	case E_LTH:
		/* TODO: currently we have no way to really ensure that
		 * e is an expression involving only string or numeric
		 * constants */
		result[0] = bool_const(expr_calc_value(e) == yes);
		result[1] = bool_const(false);
		return;
	case E_UNEQUAL:
	{
...
static void add_clause(const char *name, unsigned int nr_literals, ...)
{
	...
	if (clause_index >= max_clauses) {
		...
		assert(new_max_clauses > clause_index);
...
static void add_clauses(struct bool_expr *e, const char *fmt, ...)
{
	...
	/* TODO: This introduces a lot of unit clauses. In the future we
	 * may want to simplify before passing these clauses to the SAT
	 * solver. */
...
static bool build_prompt_visibility_clauses(struct property *prop)
{
	...
	if (prop->visible.expr) {
		expr_to_bool_expr(prop->menu->sym, prop->visible.expr, e);
	} else {
		e[0] = bool_const(true);
		e[1] = bool_const(false);
	}
...
static bool build_select_clauses(struct symbol *sym, struct property *prop)
{
	...
	/* t2 encodes whether this particular select is active,
	 * i.e. whether the menu symbol is enabled and the selecting
	 * prop is visible. */
	assert(sym->sat_variable <= nr_sat_variables);
	t2 = bool_and_put(bool_var(sym->sat_variable), condition[0]);
	bool_put(condition[1]);

	/* The selected expression must be a bool/tristate symbol */
	assert(prop->expr->type == E_SYMBOL);
	selected_sym = prop->expr->left.sym;

	/* Sometimes the symbol hasn't been defined. */
	assert(selected_sym->type == S_UNKNOWN || selected_sym->type == S_BOOLEAN || selected_sym->type == S_TRISTATE);

	/* Update the selected symbol's 'selected_expr' to reflect that this
	 * symbol may have selected it. */
	if (selected_sym->type != S_UNKNOWN)
		selected_sym->selected_expr = bool_or_put(selected_sym->selected_expr, bool_get(t2));

	symbol_to_bool_expr(selected_sym, selected_sym_expr);

	t3 = bool_dep_put(t2, bool_and_put(
		bool_dep_put(bool_var(sym->sat_variable + 1), bool_get(selected_sym_expr[0])),
		bool_dep_put(bool_and_put(bool_var(sym->sat_variable), bool_not_put(bool_var(sym->sat_variable + 1))),
			bool_and_put(bool_get(selected_sym_expr[0]), bool_not_put(bool_get(selected_sym_expr[1]))))
	));
	bool_put(selected_sym_expr[0]);
	bool_put(selected_sym_expr[1]);
...
static bool build_default_clauses(struct symbol *sym)
{
	...
	for_all_defaults(sym, prop) {
		...
		/* This should be the "if" part of the expression. However,
...
void satconfig_check_config(void)
{
	...
	if (nr_changed) {
		...
		fprintf(stderr, "no symbols differ from their SAT assignments; perfect!\n");
	}
}
...
void satconfig_init(const char *Kconfig_file, bool randomize)
{
	...
	/* First read .config to initialise values for non-boolean (and non-
	 * tristate) variables. In the future we may want to read these from
	 * .satconfig as well. */
...
unsigned int satconfig_core_size;
const char **satconfig_core;
...
bool satconfig_solve(void)
{
	...
	int sat = picosat_sat(-1);
	if (sat != PICOSAT_SATISFIABLE) {
		/* Extract a textual description of the unsat core */
		unsigned int i;
		unsigned int nr_core_clauses;
		...
		satconfig_core = calloc(nr_core_clauses, sizeof(*satconfig_core));

		nr_core_clauses = 0;
		for (i = 0; i < picosat_added_original_clauses(); ++i) {
			if (picosat_coreclause(i))
				satconfig_core[nr_core_clauses++] = clauses[i];
		}

		satconfig_core_size = nr_core_clauses;
		return false;
	}
	...
	for_all_symbols(i, sym) {
		...
		/* This causes _all_ symbols to be written out in the
		 * .config, even if those symbols are not visible. We may
		 * want to revisit this in the future in order to be
		 * consistent with oldconfig, menuconfig, etc. */
